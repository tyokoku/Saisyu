 ...
 *3Dオブジェクトのレンダリング方法を定義する、頂点、面、その他の属性の集まりです。
...
public class Mesh implements Closeable {
   ...
   * behave.
   * {@link VertexBuffer}のデータがどのように解釈されるかを決定します。
   * プリミティブの動作については、
   * <a href="https://www.khronos.org/opengl/wiki/Primitive">こちら</a>を参照してください。
   */
   ...
   *与えられた {@link IndexBuffer} と {@link VertexBuffer} のデータは確定する必要はなく、
   * それぞれの {@code set() } メソッドを使用して {@link Mesh} の寿命が尽きるまで自由に変更することができます。
   * それぞれの{@code set()}メソッドを使用して{@link Mesh}の寿命まで自由に変更できます。
   *
   ...
   * メッシュを描画します。
   * 低レベルのOpenGLコードを使用している場合を除き、直接これを呼び出さないでください。
   * 代わりに {@link SampleRender#draw} を使用してください。
   */
